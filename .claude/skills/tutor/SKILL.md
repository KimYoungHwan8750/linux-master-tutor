...
3. **선택지 description은 반드시 `" "` (공백)으로 설정** — 설명 텍스트가 정답을 유추하게 하므로 아예 표시하지 않는다
4. 사용자는 모를 때 **"Other" 버튼**으로 "모름"을 입력할 수 있다 (별도 모름 선택지 불필요)
5. 채점 결과는 **문제 원문 전체**를 표시한다 (요약/축약 금지):
   ```
   **Q1. ✅** 리눅스 부팅 시 가장 먼저 실행되어 PID 1번을 가지는 프로세스는?
   → 내 답: init/systemd | 정답: init/systemd

   **Q2. ❌** 클라이언트 요청이 있을 때만 프로세스를 생성하는 데몬 방식은?
   → 내 답: systemd | 정답: xinetd
   ```
6. 오답에 대한 **상세 설명** 제공:
   - **줄임말은 반드시 풀네임 병기**: NFS (Network File System), SMB (Server Message Block) 등
   - **명령어 관련 오답은 실제 명령어 예시** 포함 (코드블록 사용)
   - 혼동하기 쉬운 개념은 **비교 형식**으로 명확히 구분
   - 관련 설정 파일/경로가 있으면 함께 제시
6. **배치 간 진행 표시** 후 즉시 다음 배치 진행 (사용자 확인 불필요):
